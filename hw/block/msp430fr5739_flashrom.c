...
#define TYPE_MSP430FR5739_FLASHROM "msp430fr5739_flashrom"
#define MSP430FR5739_FLASHROM(obj) OBJECT_CHECK(MSP430FlashRomState, (obj), TYPE_MSP430FR5739_FLASHROM)

#define MSP430FR5739_FLASHROM_CONFIG_BASE       (0x0128)
#define MSP430FR5739_FLASHROM_CONFIG_WORDSIZE   (3)
#define FCTL1 (0)
#define FCTL2 (1)
#define FCTL3 (2)
...
typedef struct 
{
    ...
    hwaddr config_base;
    int32_t config_size;
    uint16_t r[MSP430FR5739_FLASHROM_CONFIG_WORDSIZE];
...
{
    MSP430FlashRomState *device = (MSP430FlashRomState *)opaque;
    qemu_log("MSP430FR5xxx FlashRom Config: Handling Memory Write\n");
    offset >>= 1;
...
static uint64_t flashrom_config_read(void *opaque, hwaddr offset, unsigned size)
{
    ...
    offset >>= 1;
    qemu_log("MSP430FR5xxx FlashRom Config: Handling Memory Read\n");
    return d->r[offset];
}
 ...
 ******************************************************************/
static void msp430fr5739_flashrom_reset(DeviceState *dev)
{
    MSP430FlashRomState *s = MSP430FR5739_FLASHROM(dev);
 ...
 */
static int msp430fr5739_flashrom_init(SysBusDevice *dev)
{
    ...
    // FlashRom device. 
    s->config_size = (MSP430FR5739_FLASHROM_CONFIG_WORDSIZE * 2);
    s->config_base = MSP430FR5739_FLASHROM_CONFIG_BASE;
 ...
 */
static void msp430fr5739_flashrom_class_init(ObjectClass *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);
    SysBusDeviceClass *sdc = SYS_BUS_DEVICE_CLASS(klass);
    sdc->init = msp430fr5739_flashrom_init;
    dc->reset = msp430fr5739_flashrom_reset;
...
static const TypeInfo msp430_flashrom_info = {
    .name          = TYPE_MSP430FR5739_FLASHROM,
    .parent        = TYPE_SYS_BUS_DEVICE,
    .instance_size = sizeof(MSP430FlashRomState),
    .class_init    = msp430fr5739_flashrom_class_init,
};
 ...
 */
static void msp430fr5739_flashrom_register_types(void)
{
...
type_init(msp430fr5739_flashrom_register_types)
